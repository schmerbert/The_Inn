...
Files are named `NN-YYYY-MM-DD-slug.md` where `NN` is read order. Each file's
**Source** line keeps the original transcript id (matches `logs/.source/*.jsonl`).

| File | Builder | Pass | Notes |
|------|---------|------|-------|
| `01-2026-07-02-prebuild-mapping.md` | Claude | 1 | Main mapping — PREBUILD, inn cast, scribe/steward, forest |
| `02-2026-07-02-prebuild-mapping-compacted.md` | Claude | 1 | Continuation of mapping after context compaction (summary + tail) |
| `03-2026-07-02-marble-alignment.md` | Cursor | 1 | Familiarize repo; "same marble?" alignment; motion / exemplar compare |
| `04-2026-07-03-map-survey-first-push.md` | Fable 5 | 1 | MAP/PYRAMID cold read, burial (test 6), first-push inventory |
...
```bash
python tools/archive_session.py path/to/<uuid>.jsonl logs/NN-YYYY-MM-DD-short-slug.md --note "Pass N — short label"
```
